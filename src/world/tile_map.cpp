...
TileMap::TileMap(const std::string &file_name)
{
    ...
    std::vector<int> platforms = get_data("platform_layer");
    ...
    setup_tiles(platforms, m_platforms);
...
const std::vector<sf::FloatRect> &TileMap::get_platforms() const
{
    return m_platforms;
}
...
void TileMap::setup_positions(const std::vector<int> &data, std::vector<std::pair<uint32_t, sf::Vector2f>>& positions)
{
    for (int i = 0; i < total_tiles_y; ++i)
    {
        for (int j = 0; j < total_tiles_x; ++j)
        {
            ...
            if (entity_id != 0)
            {
                float x_pos = j * tile_width * tile_scale + tile_width * tile_scale / 2.f;
                float y_pos = (i + 1) * tile_height * tile_scale;
